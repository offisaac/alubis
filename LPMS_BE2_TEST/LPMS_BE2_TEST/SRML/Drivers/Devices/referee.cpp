...
{
	uint8_t pitch_angle_str[] = "pitch angle: ";
	uint8_t pitch_angle_name[] = "1p0";
	
	/* 绘制英雄pitch角度框架 */
	Draw_Char(_layer, start_x, start_y, pitch_angle_name, pitch_angle_str, sizeof(pitch_angle_str), size, _color, ADD_PICTURE);

	/* 绘制数字 */
	pitch_angle_name[2] = '1';
	memcpy(data_pack, (uint8_t*)float_drawing(_layer, ADD_PICTURE, start_x + sizeof(pitch_angle_str)*20, start_y, size, size/10, _color, 0.0f, pitch_angle_name), DRAWING_PACK);
 ...
 * @note 
 */
void referee_Classdef::Hero_PitchAng_Update(float ang_cur, uint8_t _layer, uint16_t start_x, uint16_t start_y, uint16_t size, colorType_e _color)
{
	static uint8_t pitch_angle_name[] = "1p1";
	
	/* 绘制数字 */
	memcpy(data_pack, (uint8_t*)float_drawing(_layer, MODIFY_PICTURE, start_x + 12*20, start_y, size, size/10, _color, ang_cur, pitch_angle_name), DRAWING_PACK);
	pack_send_robotData(Drawing_1_ID, robot_client_ID.client, (uint8_t*)data_pack, DRAWING_PACK);
}
